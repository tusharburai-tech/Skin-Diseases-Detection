"""
backend/app.py — Full standalone Flask app (backwards-compatible).

Gunicorn runs:  gunicorn main:app --config gunicorn.conf.py
This file re-exports that same `app` object so any import path works:
...
# ── Resolve project root (works whether imported as backend.app or run directly) ──
